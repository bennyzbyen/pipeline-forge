...
Use this reference when generated report code touches local `gateway/`, `fs`, or `hbase` packages.
...
- Keep `gateway/`, `fs/`, and `hbase/` as fixed platform packages. Put report-specific logic in `data_utils/` and `params_configs/`.
- For local fake-runtime verification, prefer injected `gateway_client`, `hbase_client`, and `fs_client` paths over live Gateway access.

## FS Patterns

- Default download pattern: `exists(remote)` -> `copy_to_local(remote, local)` -> Pandas reads local file -> delete the local temp file.
- Default upload/evidence pattern: write a local temp CSV/TSV/GZIP file -> `copy_from_local(local, remote, overwrite=True)` -> delete the local temp file.
- `copy_to_local` parameter order is remote first, local second. `copy_from_local` is local first, remote second.
- Use `listdir(dir)` only for documented data directories; missing remote paths should produce empty DataFrames or explicit confirmation items based on report requirements.
...
- Let HBase wrappers manage their own FS export/import/delete temp directories; do not hand-build `fs_root_dir/export`, `fs_root_dir/import`, or `fs_root_dir/delete` paths in report business code.
...
- Period/range reads normally use `row_start=period_or_start`, `row_stop=f"{period_or_end}Z"`.
...
- If a source read returns no rows, preserve the expected output columns so downstream joins and summaries stay deterministic.
...
- Generated COT/report rowkeys often concatenate configured columns in order, then prefix the concatenated key with its last character.
...
- Never guess rowkey fields. Keep uncertain rowkeys in `params_configs/rowkey_config.py` and confirmation notes.
