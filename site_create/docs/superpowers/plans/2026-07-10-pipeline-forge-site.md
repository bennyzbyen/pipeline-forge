...
- The plugin version displayed by the site is `1.0.2`.
...
- [ ] **Step 3: Write the failing content regression test**
  ...
  describe("PipelineForge product facts", () => {
    it("publishes the exact current plugin identity and capability set", () => {
      expect(PLUGIN_VERSION).toBe("1.0.2");
...
- [ ] **Step 3: Perform final browser QA**
  ...
  Check all four target viewports, every navigation anchor, primary and secondary CTA, mobile menu, copy feedback, keyboard focus, reduced-motion, and JavaScript-disabled content. Confirm all seven capabilities and version `1.0.2` appear.
